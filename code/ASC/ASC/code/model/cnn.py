...
MODEL_ROOT = '/home/guwenqi/dataset/ASC/model/cnn/classical'
...
def CNN():
  ...
  with tf.variable_scope('cnn/inputs'):
    w = tf.get_variable('w', shape=[7, 7, 1, 8], initializer=tf.contrib.layers.xavier_initializer())
    b = tf.get_variable('b', shape=[8], initializer=tf.zeros_initializer())
    cnn_inputs = tf.nn.relu(tf.nn.conv2d(inputs_network, w, strides=[1, 1, 1, 1], padding='SAME') + b)
  cnn_outputs = tf.layers.max_pooling2d(cnn_inputs, pool_size=2, strides=2, padding='SAME')

  channels = [8, 16, 16, 32]
  for channel in channels:
    cnn_outputs = conv2d_bn_relu_pool_drop_layer(cnn_outputs, channel, 0.5, is_training)


  # I get this 2048 through sees.run(tf.shape(fnn_inputs))
  fnn_inputs = tf.reshape(cnn_outputs, [batches, 256])

  # fnn layers
  fnn_1 = fnn_bn_relu_drop_layer(fnn_inputs, 256, 0.5, is_training, 1)
