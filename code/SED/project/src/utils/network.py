...
def multiLayerConv2d(num_layers, inputs, kernels_size, input_channel, outchannels, pool_strides, name):
  """
  ...
  params:
    ...
    input_channel : channel of the inputs
  ...
  channels = [input_channel] + outchannels
  for i in range(num_layers):
    inputs = conv2d(inputs, kernels_size[i], channels[i], channels[i+1], pool_strides[i], name + '_' +str(i+1))

  return inputs

def FNNLayer(inputs, last_layer_units, units, keep_prob, name):
  """
  this function is used to build a one layer FNN
  params:
    inputs : tf.Tensor, inputs of this layer
    last_layer_units : units number of last layer
  ...
  """
  w = tf.get_variable(name=name+'/w', shape=[last_layer_units, units], initializer=tf.contrib.layers.xavier_initializer())
  b = tf.get_variable(name=name+'/b', shape=[int(units)], initializer=tf.zeros_initializer())
  h = tf.nn.relu(tf.add(tf.matmul(inputs, w), b))
  h_drop = tf.nn.dropout(h, keep_prob)

  return h_drop

def multiFNNLayers(num_layers, inputs, inputs_feature_dimension, units_list, keep_prob_list, name):
  """
  this function is used to construct multi layers FNN
  params:
    num_layers : number of layers
    inputs : tf.Tensor of tf.placeholder
    inputs_feature_dimension : the last dimension(feature dimension) of inputs
  ...
  units_list = [inputs_feature_dimension] + units_list
  for i in range(num_layers):
    inputs = FNNLayer(inputs, units_list[i], units_list[i+1], keep_prob_list[i], name + '_' + str(i+1))
...
def BiRNN(num_layers, units_list, keep_prob, cell_type, **kwargs):
  ...
  if len(units_list) != num_layers:
    ...
    sys.exit(0)
  if len(keep_prob) != num_layers:
    print("Fatal Error at function multiFNNLayers, length of argv keep_prob must be same as num_layers")
    sys.exit(0)

  fw_cells = []
  bw_cells = []

  if cell_type == tf.contrib.rnn.BasicRNNCell or cell_type == tf.contrib.rnn.BasicLSTMCell or cell_type == tf.contrib.rnn.GRUCell:
    for i in range(num_layers):
      cell = cell_type(units_list[i], reuse=kwargs.get("reuse", None))
      cell_drop = tf.contrib.rnn.DropoutWrapper(cell, input_keep_prob=keep_prob[i], output_keep_prob=keep_prob[i])
